...
export default function Singlepost(props) {
 ...
 const commentt=useRef();
    ...
    //sending comments
    const sendComment=()=>{
         secaxios.post("http://localhost:8000/api/comments/addcomment",{
    ...
    const recieveComments=async()=>{
         axios.get(`http://localhost:8000/api/comments/getcomments?id=${props.id}`).then((res)=>{
    ...
    const deletecomment=async(deleteme)=>{
        try{
         await secaxios.delete(`http://localhost:8000/api/comments/deletecomment`,{data:{
             postid:deleteme,
             username:props.curruser,
         }});
