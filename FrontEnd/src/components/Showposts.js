...
const updatestateliked=(index,newobject)=>{
   ...
   //headers
    ...
    const recieve=(async()=>{
        ...
        thiaxios.get("http://localhost:8000/api/posts/theposts").then((res)=>{
                setstate(res.data)
            },(err)=>{
                console.log(err);
            })
    ...
    const sendPost=(()=>{
            ...
            if(postt.current.value!=""){
             secaxios.post("http://localhost:8000/api/posts/setpost",{
    ...
    const deletepost =async(postid)=>{
        try{
        var a=await secaxios.post("http://localhost:8000/api/posts/deletepost",{
            id:postid.id,
    ...
    //        )){
        ...
        if(already==false){
         secaxios.post("http://localhost:8000/api/posts/addlikes",{
             postid,
             id:user.id,
             username:user.name,
         }).then((res)=>{
